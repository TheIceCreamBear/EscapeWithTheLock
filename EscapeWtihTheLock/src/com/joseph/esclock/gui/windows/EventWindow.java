...
public class EventWindow extends Window {
	...
	private final int numberVisibleLines;
	
	private static EventWindow instance;
	
	public EventWindow(int old) {
		this(0, 0, 500, ScreenReference.HEIGHT - 1);
	}
	
	public EventWindow() {
		this(0, 835, ScreenReference.WIDTH - 1, 200);
	}
	...
	public EventWindow(int x, int y, int width, int height) {
		...
		this.visible = false;
		this.charactersPerLine = this.width / (ScreenReference.CHAR_WIDTH) - 1;
		this.numberVisibleLines = this.height / (ScreenReference.CHAR_HEIGHT) - 1;
		System.out.println(this.charactersPerLine);
		System.out.println(this.numberVisibleLines);
	...
	public void drawUpdateableElements(Graphics g, ImageObserver observer) {
		...
		for (int i = 0; i < events.size() && i < numberVisibleLines; i++) {
